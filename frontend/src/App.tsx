...
const RUN_HISTORY_INITIAL_LIMIT = 8
...
export default function App() {
  ...
  const currentIncidents = runIncidents
  const showSynthetic = Boolean(selectedRun && isSynthetic(selectedRun))
  const replayActive = mode === 'REPLAY' && selectedRun?.mode === 'REPLAY'
  return <div className="app-shell"><header className="topbar"><div className="brand"><span className="brand-mark">AT</span><div><b>AirTrace</b><small>Environmental intelligence</small></div></div><nav className="mode-nav"><button className={mode === 'LIVE' ? 'active' : ''} onClick={() => { setMode('LIVE'); setShowReplayPanel(false); setSelectedIncident(null); setSensor(null); setHistoricalSensors(null) }}>LIVE <span className="live-dot" /></button><button className={mode === 'REPLAY' ? 'active' : ''} onClick={() => { setMode('REPLAY'); setShowReplayPanel(true); setSelectedIncident(null); setSensor(null) }}>REPLAY</button></nav><div className="top-status">{replayActive ? <><span>Historical window</span><span>{formatReplayWindow(selectedRun?.analysis_start_utc, selectedRun?.analysis_end_utc)}</span><span className="loaded-status">Loaded just now</span></> : <><span><i className="status-dot" />{live?.sensors.status === 'OK' ? 'Connected' : 'Degraded'}</span><span>Updated {live ? formatAge((Date.now() - new Date(live.as_of_utc).getTime()) / 60000) : '—'}</span></>}<button className="icon-button" title="System status" onClick={() => setShowStatus(true)}>⚙</button></div></header>{error && <div className="error-banner">{error}<button onClick={() => setError(null)}>Dismiss</button></div>}{showSynthetic && <div className="demo-banner"><b>DEMO — SYNTHETIC VALIDATION</b><span>Displayed data is the existing synthetic TRACE_COMPLETE scenario, not a real pollution event.</span></div>}{mode === 'REPLAY' && showReplayPanel ? <ReplayPanel runs={runs} onRun={submitReplay} onSelectRun={(run) => { void getRun(run.run_id).then((detail) => { setSelectedRun(run); setRunIncidents(runSummaries(detail)); setHistoricalSensors(run.mode === 'REPLAY' ? (detail.historical_sensors ?? []) : null); setSelectedIncident(detail.incidents[0] ?? null); setSensor(null); setMode('REPLAY'); setShowReplayPanel(false) }).catch(() => setError('Run artifact unavailable')) }} job={job} /> : <main className="workspace"><IncidentRail incidents={currentIncidents} selected={selectedIncident?.incident.incident_id ?? null} onSelect={selectIncident} synthetic={showSynthetic} replay={replayActive} /><MapView live={live} run={replayActive ? selectedRun : null} sensorData={replayActive ? historicalSensors : null} synthetic={showSynthetic} incident={selectedIncident} layers={layers} onToggleLayer={(layer) => setLayers((current) => ({ ...current, [layer]: !current[layer] }))} onSelectSensor={setSensor} /><Inspector incident={selectedIncident} sensor={sensor} run={replayActive ? selectedRun : null} /></main>}<footer className="footer"><span>AirTrace identifies evidence-supported candidate source regions.</span><span>It does not determine legal responsibility or prove a specific facility caused an event.</span><div className="freshness"><span>PM2.5 {status?.pm25?.freshness?.fresh ?? 0} / {status?.pm25?.freshness?.total ?? 0} fresh</span><span>Weather {status?.weather?.summary ? 'available' : 'unavailable'}</span></div></footer>{showStatus && <div className="status-overlay" role="dialog" aria-label="System status"><div className="status-drawer"><div className="drawer-head"><div><span className="eyebrow">DATA / SYSTEM</span><h2>System status</h2></div><button className="icon-button" onClick={() => setShowStatus(false)} aria-label="Close status">×</button></div><div className="status-item"><span>PM2.5 recorder</span><b>{status?.pm25?.database_status ?? 'Loading'}</b><small>{status?.pm25?.freshness?.fresh ?? 0} / {status?.pm25?.freshness?.total ?? 0} fresh · median {status?.pm25?.median == null ? '—' : `${Number(status.pm25.median).toFixed(1)} µg/m³`}</small></div><div className="status-item"><span>Weather recorder</span><b>{status?.weather?.database_status ?? 'Loading'}</b><small>{status?.weather?.summary ? `${status.weather.summary.station_count} stations · ${Number(status.weather.summary.speed_mps).toFixed(1)} m/s` : 'No current wind field'}</small></div><div className="status-item"><span>CEMS context</span><b>{status?.cems?.metadata?.dataset_complete === false ? 'PARTIAL / UNAVAILABLE' : 'Not loaded'}</b><small>Supporting context only; no completeness claim.</small></div><div className="status-item"><span>FIRMS</span><b>{status?.firms?.configured ? 'configured: true' : 'configured: false'}</b><small>Credential value is never returned to the UI.</small></div><p className="muted">Recorder databases are read through short-lived, read-only connections. A lock is surfaced as DB_LOCKED.</p></div></div>}</div>
}
